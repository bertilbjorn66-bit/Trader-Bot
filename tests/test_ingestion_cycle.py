...
def test_ingestion_is_idempotent_and_checkpoints(tmp_path: Path) -> None:
    ...
    with IngestionStore(tmp_path / "ingestion.duckdb") as store:
        first = ingest_series(provider, store, provider_name="fake", asset_class="FOREX", instrument=1, timeframe=Timeframe.ONE_MINUTE, offer_side=OfferSide.BID, start=start, end=end, observed_at=end)
        second = ingest_series(provider, store, provider_name="fake", asset_class="FOREX", instrument=1, timeframe=Timeframe.ONE_MINUTE, offer_side=OfferSide.BID, start=start, end=end, observed_at=end)
        assert first["received"] == 2
        assert first["stored"] == 2
        assert second["received"] == 2
        assert second["stored"] == 0
        assert store.count_series(provider="fake", asset_class="FOREX", instrument=1, timeframe=Timeframe.ONE_MINUTE.value, offer_side=OfferSide.BID.value) == 2
        checkpoint = store.checkpoint(provider="fake", asset_class="FOREX", instrument=1, timeframe=Timeframe.ONE_MINUTE.value, offer_side=OfferSide.BID.value)
...
def test_cycle_isolates_series_failures(tmp_path: Path) -> None:
    ...
    plans = (SeriesPlan("FOREX", 1, Timeframe.ONE_MINUTE, OfferSide.BID, start), SeriesPlan("FOREX", 2, Timeframe.ONE_MINUTE, OfferSide.BID, start))
...
def test_invalid_provider_response_fails_closed() -> None:
    start = datetime(2026, 8, 1, tzinfo=timezone.utc)
    request = DataRequest(instrument=1, timeframe=Timeframe.ONE_MINUTE, start=start, end=start + timedelta(minutes=1), offer_side=OfferSide.BID)
    wrong = _bar(2, start)
    try:
