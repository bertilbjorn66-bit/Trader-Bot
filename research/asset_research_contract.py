...
class ResearchContract:
    """Rules that every asset-specific research engine must satisfy."""
    ...
    def validate_profile(self, profile: InstrumentProfile) -> None:
        ...
        if not self.empirical_only:
            raise ValueError("multi-asset research contracts must remain empirical-only")
        if self.allow_future_labels:
            raise ValueError("future labels cannot be permitted")
        if self.allow_confirmation_learning:
            raise ValueError("confirmation learning cannot be permitted")
        if not self.require_transaction_costs:
            raise ValueError("transaction-cost modeling is mandatory")
        if not self.require_liquidity_filter:
            raise ValueError("liquidity filtering is mandatory")
...
def contract_for(profile: InstrumentProfile) -> ResearchContract:
    """Build conservative research requirements from the instrument contract."""
