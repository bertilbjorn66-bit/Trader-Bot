...
def inverted_profit_factor(win_rate: float, avg_win: float, avg_loss: float) -> float | None:
    """PF of the hypothetical sign-inverted directional rule."""
...
def main() -> None:
    ...
    for pair_report in pairs:
        ...
        for horizon_text, horizon in pair_report["horizons"].items():
            ...
            inverted_pf = inverted_profit_factor(win_rate, avg_win, avg_loss)
            baseline.append(
                {
                    "pair": pair,
                    ...
                    "inverted_profit_factor": inverted_pf,
                }
            )
    ...
    robust = [cell for cell in survivors if cell["win_rate_ci_low"] is not None and cell["win_rate_ci_low"] > 0.45]
    ...
    for cell in eligible:
        ...
        # Recover aggregate winning/losing movement in pips from the report's sufficient statistics.
        # PF itself is already unitless; these sums are only used to aggregate across pairs.
        result = next(
    ...
    output = {
        ...
        "robust_survivors": sorted(robust, key=lambda row: (row["profit_factor"], row["expectancy_pips"]), reverse=True),
        ...
        "interpretation": {
            ...
            "conditional": "The completed report contains conditional pockets of positive expectancy, most notably high_volatility_range on EUR/JPY and GBP/JPY at horizons 3-6, plus several year-specific USD/JPY and EUR/USD cells.",
            "robustness": "These are discovery signals, not validated strategies. Grouping by regime/session/year creates multiple comparisons, and the report lacks per-target observations needed for bootstrap confidence intervals or true multiple-testing control.",
            "analogue_quality_limit": "The final artifact retains average analogue count and probability calibration but not per-target analogue distances, agreement bins, or thresholded predictions. Those dimensions cannot be audited from this artifact without changing the research output schema.",
