...
def load_feed_bars(path: Path) -> list[dict[str, float | int]]:
...
def _market_bars(rows: list[dict[str, float | int]]) -> tuple[tuple[MarketBar, ...], tuple[MarketBar, ...]]:
    bid: list[MarketBar] = []
    ask: list[MarketBar] = []
    for row in rows:
        ...
        bid.append(
            MarketBar(
                ...
                open=Decimal(str(row["bid_open"])),
                high=Decimal(str(row["bid_high"])),
                low=Decimal(str(row["bid_low"])),
                close=Decimal(str(row["bid_close"])),
            )
        )
        ask.append(
            MarketBar(
                ...
                open=Decimal(str(row["ask_open"])),
                high=Decimal(str(row["ask_high"])),
                low=Decimal(str(row["ask_low"])),
                close=Decimal(str(row["ask_close"])),
            )
        )
...
def main() -> None:
    ...
    payload = {
        ...
        "source": "Dukascopy public datafeed via dukascopy-node; native m5 aggregated to 10m",
