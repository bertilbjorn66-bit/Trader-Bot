...
def _stat_for(
    local: dict[tuple[Any, ...], Stat], global_stats: dict[tuple[str, int], Stat],
    name: str, horizon: int, pair: str, regime: str, session: str,
) -> Stat:
...
def route(
    votes: dict[str, tuple[int, float]], local: dict[tuple[Any, ...], Stat], global_stats: dict[tuple[str, int], Stat],
    horizon: int, pair: str, regime: str, session: str,
    ...
    for position in range(history_states, len(states) - max(DEFAULT_HORIZONS), sample_stride):
        ...
        for horizon in DEFAULT_HORIZONS:
            ...
            if lead is not None:
                ...
                value = net_move(outcome.return_abs, costs) / PAIR_PIP[pair]
                decisions.append({
                    "timestamp": target.timestamp.isoformat(), "pair": pair, "horizon": horizon,
                    "regime": f"regime:{regime}", "session": session, "strategy": lead,
        ...
        # Crucial causal boundary: learn only after ALL horizons for this target timestamp are decided.
...
def run(input_dir: Path, sample_stride: int, history_states: int, slippage_pips: float, commission_pips: float) -> dict[str, Any]:
    ...
    for pair in PAIR_TO_SYMBOL:
        ...
        records = evaluate_pair(pair, rows, sample_stride, history_states, costs)
    ...
    stress_hard = stress["1.5"]
    preliminary = bool(base["n"] >= 100 and base["expectancy_pips"] and base["expectancy_pips"] > 0 and (base["profit_factor"] or 0) > 1 and stress_hard["expectancy_pips"] and stress_hard["expectancy_pips"] > 0 and (stress_hard["profit_factor"] or 0) > 1 and (positive_years / len(year_reports) if year_reports else 0.0) >= 0.67)
    return {
        "status": "ADAPTIVE_ENSEMBLE_V3_COMPLETED", "empirical": True, "synthetic": False,
        "strategy_set": list(STRATEGIES), "decision_count": len(ordered), "pair_summaries": pair_summaries,
        "overall": summary(ordered), "chronological_discovery_60pct": summary(discovery),
        "chronological_confirmation_40pct": base, "stress": stress,
        "year_stability": {"years": year_reports, "positive_year_count": positive_years, "observed_year_count": len(year_reports), "positive_year_fraction": positive_years / len(year_reports) if year_reports else 0.0},
        "design": {
            "multi_expert_consensus": True, "current_vs_past": True, "online_learning": True,
            "context_dimensions": ["pair", "regime", "session", "horizon", "spread_anomaly"],
            "same_timestamp_cross_horizon_leakage_blocked": True,
            "analogue_future_window_must_end_before_target": True,
            "abstain_on_disagreement": True, "abstain_on_weak_edge": True, "live_orders": False,
        },
        ...
        "warning": "This is non-live empirical research. No historical system can guarantee perfect forecasting or future profitability.",
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Adaptive multi-expert, context-aware, leakage-safe empirical research v3.")
