...
def run_cycle(
    ...
    *,
    ...
    for plan in plans:
        ...
        if start >= now:
            outcomes.append(SeriesOutcome(plan, "CURRENT", start, now))
            continue
        try:
            ...
            outcomes.append(SeriesOutcome(plan, "FAILED", start, now, reason=f"{type(exc).__name__}: {exc}"))
            continue
        outcomes.append(
            SeriesOutcome(
                plan,
                "UPDATED" if result["received"] else "NO_DATA",
                start,
                now,
